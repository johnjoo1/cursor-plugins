...
---
<!-- Adapted from pstack/skills/show-me-your-work (pstack by Lauren Tan, MIT). Modified for Claude Code: see rigor/README.md. -->
# Decision log
...
At the end of the run, before handing back, check the log told the truth. Walk each row against what actually happened in this session, and against the artifacts the rows point at:
...
Before handing back, spawn a reviewer that did not do the work. Self-review is not a substitute; the point is eyes you cannot bring yourself. Use `subagent_type: "Explore"` on a different tier from the one that did the work, or a cross-vendor reviewer when one is configured. Hand it the trail and the artifacts each row cites, since it cannot see this conversation. It flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.

**The caveat that matters here.** A different-tier Claude reviewer shares most of this agent's priors, so it will miss roughly what this agent missed. That makes it a useful check on *evidence hygiene* (does each row resolve, is anything unverified) and a weak check on *judgment* (was the whole approach wrong). Say which you got. A cross-vendor reviewer, being genuinely decorrelated, is worth more for the second and is the one case where the trail plus artifacts is enough context for a one-shot call to do real work.

- Decisions logged with weak or absent evidence.
- Verification steps skipped, or claimed with no artifact backing the claim.
- Choices that look risky in hindsight (premature, scope-creeping, papering over a symptom).
- Gaps the user would otherwise miss on a casual skim.

Every reply for a run that produced a trail ends with an "Attention" section. Lead with the reviewer on its own line (`reviewed by <model>`, and say plainly when it was same-family), then list each flag pointing to specific rows or moments. "No flags" is a valid value; the model name is not. The self-audit asks if the log told the truth; this asks what the user should still scrutinize even when it did.
