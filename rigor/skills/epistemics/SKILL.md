...
Phrasing: explicitly speculative. "One possibility is X, but we have no direct evidence." Belongs in **Competing hypotheses**, listed alongside the other explanations that fit equally well, never presented alone as though it were the answer.
...
These signal that you're interpreting, not reporting. Use them liberally in **What we can reasonably infer**.
...
Structure any evidence-based writeup in these sections, so a reader can see which tier each claim sits in without decoding your phrasing:

- **What we found.** Direct and Supported claims only, each with its citation.
- **What we can reasonably infer.** Inferred claims, hedged, with the inference chain made explicit.
- **Competing hypotheses.** Speculative readings, listed together so none reads as settled.
- **What we don't know.** The gaps, named concretely: the question, what you searched, what you searched for, what came back.

A writeup with no fourth section is the one to distrust.
