...
---
<!-- Adapted from pstack/skills/architect (pstack by Lauren Tan, MIT). Modified for Claude Code: see rigor/README.md. -->
# Architect
...
Build a real mental model of every system the new code touches. Fan out read-only explorers (`subagent_type: "Explore"`) over 2-4 distinct slices of the affected subsystems, then reconcile their findings yourself.

Naming a file isn't grounding. The bar is a traced path: from an entry point, follow the call chain through callers, callees, data flow, and type definitions until you can describe input to output with no hand-waved step. Note what is surprising or what a newcomer would get wrong.

If the design redefines ownership or layering, also establish *why* the existing shape is the way it is before overriding it, so the rationale becomes a constraint rather than a guess. Grade that history with the **epistemics** skill; an inferred rationale is not a settled one.
...
Arena owns the runner roster; see its Phase A. Because every runner is a Claude model, read Arena's Phase E caveat on convergence before treating agreement between candidates as confirmation.
...
1. Re-ground over what's been built, per Phase A. The implementation lessons enter the new design as inputs, not vibes.
